...
namespace sy::RHI
{
    ...
    {
        const auto completedFenceValue = this->GetCompletedValue();
        const auto numOfFramesInFlight = this->GetValue() - completedFenceValue;
        ...
        if (allowedNumOfSimultaneousFrames <= numOfFramesInFlight)
        {
            ...
            const auto valueToWaitFor = this->GetValue() - (allowedNumOfSimultaneousFrames - 1);
