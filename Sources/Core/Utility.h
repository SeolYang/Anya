...
namespace sy::utils
{
    ...
    namespace traits_impl
    {
        ...
        template <typename TupleT>
        struct is_tuple_contains_other_tuple : std::false_type { };

        template <typename... ElementTypes>
        struct is_tuple_contains_other_tuple<std::tuple<ElementTypes...>> :
            std::conditional_t<
            std::disjunction_v<is_tuple_impl<ElementTypes>...>,
            std::true_type,
            std::false_type>
        {
        };
    ...
    template <typename TupleT>
    using is_tuple_contains_other_tuple = traits_impl::is_tuple_contains_other_tuple<std::decay_t<TupleT>>;
    ...
    namespace _internal
    {
        ...
        static constexpr uint64 CRCTable[256] = {
    ...
    template<size_t idx>
    constexpr uint32 CRC32(const char* str)
    {
        return (CRC32<idx - 1>(str) >> 8) ^ _internal::CRCTable[(CRC32<idx - 1>(str) ^ str[idx]) & 0x000000FF];
    }

    template<>
    constexpr uint32 CRC32<size_t(-1)>(const char*)
    {
        return 0xFFFFFFFF;
    }
...
#define COMPILE_TIME_CRC32_STR(x) (anya::utils::_internal::CRC32<sizeof(x) - 2>(x) ^ 0xFFFFFFFF)
    ...
    */
    constexpr uint32 ELFHash(const char* str)
    {
        unsigned int hash = 0;
        unsigned int x = 0;
